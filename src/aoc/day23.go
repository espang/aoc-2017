...
func ParseLine(l string) (Operation, error) {
	...
	case "set":
		...
		if isKey(parts[2]) {
			...
			return SetValue{Register: register, Value: int(v)}, nil
		}
	case "sub":
		if !isKey(register) {
			return nil, fmt.Errorf("unexpected register pos %s in %q", register, l)
		...
		if isKey(parts[2]) {
			...
			return SubValue{Register: register, Value: int(v)}, nil
		}
	case "mul":
		if !isKey(register) {
			return nil, fmt.Errorf("unexpected register pos %s in %q", register, l)
		...
		if isKey(parts[2]) {
			...
			return MulValue{Register: register, Value: int(v)}, nil
		}
	case "jnz":
		...
		if isKey(parts[1]) {
			...
			return JumpValue{Value: int(v), Jump: int(jump)}, nil
		}
	default:
...
func part2(ops []Operation) {
	...
	fmt.Println("Register h:", reg.m["h"])
}
